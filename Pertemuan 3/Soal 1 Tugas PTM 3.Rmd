...
```{r}
library(readxl)
data <- read_excel("D:\\SEM 5\\MPDW\\MPDW\\Pertemuan 3\\Dataset Kuliah 3.xlsx", sheet = "data1")
str(data)
data
```
...
```{r}
fore.koyck <- dLagM::forecast(model = model.koyck, x=test$Price, h=5)
fore.koyck
...
```{r}
fore.dlm <- dLagM::forecast(model = model.dlm, x=test$Price, h=5)
fore.dlm
...
```{r}
#peramalan dan akurasi
fore.dlm2 <- dLagM::forecast(model = model.dlm2, x=test$Price, h=5)
...
```{r}
fore.ardl <- dLagM::forecast(model = model.ardl, x=test$Price, h=5)
fore.ardl
```
...
Dari tabel di atas, dapat terlihat bahwa nilai AIC terendah didapat ketika $p=6$ dan $q=1$, yaitu sebesar `-141.3378`. Artinya, model autoregressive optimum didapat ketika $p=6$ dan $q=1$.
